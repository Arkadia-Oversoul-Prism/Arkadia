...
A.I.S is the human identity formation step of signup. The authenticated
Firebase user remains the source of identity authority; this module projects
A.I.S into that existing user profile store and provides one longitudinal
read model for Arkadia surfaces.
"""
...
_RELATIONAL_DIMENSIONS = (
    "identity_coherence", "context_continuity", "personalization_fidelity",
    "agency_preservation", "trust_calibration", "responsiveness",
    "developmental_coherence", "relational_alignment",
)
...
def _seed_material(uid: str, seed: dict[str, Any] | None) -> str:
    return json.dumps({"uid": uid, "seed": seed or {}}, sort_keys=True, ensure_ascii=False)


def _sigil_for(uid: str, seed: dict[str, Any] | None) -> str:
    sigils = ("◈", "✦", "⬡", "◇", "✧", "◉", "⌬", "⟐", "✺", "❖")
    digest = hashlib.sha256(_seed_material(uid, seed).encode("utf-8")).digest()
    return sigils[digest[0] % len(sigils)]


def _username_for(user: dict[str, Any], stored: dict[str, Any], seed: dict[str, Any] | None, uid: str) -> str:
    existing = user.get("username") or stored.get("username")
    if isinstance(existing, str) and existing.strip():
        return _slug(existing) or f"node-{uid[:8]}"
    preferred = (stored.get("display_name") or user.get("display_name") or "node").strip()
    base = _slug(preferred) or "node"
    return f"{base}-{uid[:4].lower()}"


def _build_identity_spine(uid: str, user: dict[str, Any], stored: dict[str, Any]) -> dict[str, Any]:
    """Build the canonical longitudinal identity read model from existing stores."""
    seed = _seed_from_ais(stored)
    existing = stored.get(_SPINE_KEY) if isinstance(stored.get(_SPINE_KEY), dict) else {}
    now = datetime.now(timezone.utc).isoformat()
    preferred = stored.get("display_name") or user.get("display_name") or ""
    username = _username_for(user, stored, seed, uid)
    sigil = _sigil_for(uid, seed)

    identity = {
        "uid": uid,
        "canonical_name": user.get("display_name") or preferred,
        "preferred_name": preferred or user.get("display_name") or "Node",
        "username": username,
        "role": user.get("role") or "Authenticated Node",
        "role_sigil": user.get("role_sigil") or sigil,
    ...
    dimensions = old_index.get("dimensions") if isinstance(old_index.get("dimensions"), dict) else {}
    dimensions = {key: dimensions.get(key) for key in _RELATIONAL_DIMENSIONS}
    relational = {
        "version": 1,
    ...
    symbolic = {
        "seed_phrase": old_symbolic.get("seed_phrase") or (seed.get("seed_phrase") if seed else None),
        "seed_symbols": old_symbolic.get("seed_symbols") or ([seed.get("seed_phrase")] if seed and seed.get("seed_phrase") else []),
        "sigil_seed": old_symbolic.get("sigil_seed") or sigil,
    ...
    return {
        ...
        "identity": identity,
        "seed": {
            "created_at": seed.get("completedAt") if seed else None,
            "sigil": symbolic["sigil_seed"],
            "phrase": symbolic["seed_phrase"],
            "symbols": symbolic["seed_symbols"],
            "source": "A.I.S" if seed else None,
        },
        "orientation": {
            "current_intent": seed.get("identity", "") if seed else "",
            "direction": " · ".join(seed.get("growth", [])) if seed else "",
            "interests": seed.get("capabilities", []) if seed else [],
            "values": [],
        },
        ...
        "symbolic": symbolic,
        "continuity": {
            "projections": ["NovaNet", "ReasoMate", "Personal Codex", "Personal EchoField", "Spiral Grove", "IMS", "Encyclopedia Galactica"],
            "ims": user.get("ims_id"),
            "encyclopedia": "shared node continuity",
            "codex": "canonical longitudinal context",
            "echofield": "living identity visualization",
        },
        "provenance": {
...
async def patch_ais_profile(request: Request, user: dict = Depends(require_auth)):
    ...
    stored = load_user_profile_store(user["uid"])
    spine = _build_identity_spine(user["uid"], user, stored)
...
async def add_development_event(request: Request, user: dict = Depends(require_auth)):
    """Append a user-scoped development/evidence event without rewriting the A.I.S seed."""
    try:
        body = await request.json()
    except Exception as exc:
        raise HTTPException(status_code=400, detail="Invalid JSON body") from exc
    if not isinstance(body, dict) or not str(body.get("type", "")).strip():
        raise HTTPException(status_code=400, detail="Expected development event type")
    stored = load_user_profile_store(user["uid"])
    spine = _build_identity_spine(user["uid"], user, stored)
    event = {"type": str(body["type"])[:80], "summary": str(body.get("summary", ""))[:500], "source": str(body.get("source", "arkadia"))[:80], "recorded_at": datetime.now(timezone.utc).isoformat()}
    events = spine["capability"].setdefault("development_events", [])
    if len(events) >= 500:
        events.pop(0)
    events.append(event)
    spine["capability"]["evidence_count"] = int(spine["capability"].get("evidence_count", 0)) + 1
    spine["capability"]["last_assessed_at"] = event["recorded_at"]
    _save_spine(user["uid"], spine)
    return {"identity_spine": spine, "event": event}
...
async def add_relational_observation(request: Request, user: dict = Depends(require_auth)):
    """Record bounded relational-alignment evidence for later evaluation."""
    try:
        body = await request.json()
    except Exception as exc:
        raise HTTPException(status_code=400, detail="Invalid JSON body") from exc
    ...
    ri["status"] = "observed" if any(v is not None for v in ri["dimensions"].values()) else "seeded"
    known = [v for v in ri["dimensions"].values() if isinstance(v, int)]
    ri["score"] = round(sum(known) / len(known)) if known else None
    ri["confidence"] = round((len(known) / len(_RELATIONAL_DIMENSIONS)) * 100)
    ri["last_evaluated_at"] = datetime.now(timezone.utc).isoformat() if known else ri.get("last_evaluated_at")
    _save_spine(user["uid"], spine)
    return {"identity_spine": spine}
