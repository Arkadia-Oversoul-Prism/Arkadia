"""Cross-layer SG-02-FE and SG-03 frontend contract tests.
...
def test_activity_surface_is_downstream_of_learning_path() -> None:
    chamber = CHAMBER.read_text(encoding="utf-8")
    assert "learningPath={learningPath}" in PAGE.read_text(encoding="utf-8")
    assert "activities={activities}" in PAGE.read_text(encoding="utf-8")
    assert "GroveLearningActivity" in chamber
    assert 'data-testid="learning-activity-work-surface"' in chamber


def test_activity_draft_persistence_is_local_and_not_evidence() -> None:
    chamber = CHAMBER.read_text(encoding="utf-8")
