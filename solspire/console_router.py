...
async def require_project_owner(project_id: str, user: dict = Depends(require_auth)) -> dict:
    """Resolve the project and enforce caller ownership."""
...
# ── Canonical /run orchestration ───────────────────────────────────────────

def _execute_run(body: RunRequest, user: dict[str, Any]) -> dict[str, Any]:
    """Execute the single canonical SolSpire run pipeline.

    Project context is deliberately not part of orchestration. The project
    route delegates here and adds only its existing activity-log side effect.
    """
...
async def run_request(body: RunRequest, user: dict = Depends(require_auth)) -> dict[str, Any]:
    """Route a natural language request through the canonical run pipeline."""
...
async def list_provider_keys(provider: str | None = None, user: dict = Depends(require_auth)) -> dict[str, Any]:
    from solspire.provider_manager import get_manager
    m = get_manager()
    return {"keys": m.list_keys(provider), "models": m.get_models(), "auto_fallback": m.get_auto_fallback()}
...
async def activate_provider_key(key_id: str, user: dict = Depends(require_auth)) -> dict[str, Any]:
    ...
    if not entry:
        raise HTTPException(status_code=404, detail="Key not found")
    return {"ok": m.set_active_key(entry["provider"], key_id)}
...
async def console_status(user: dict = Depends(require_auth)) -> dict[str, Any]:
    ...
    from solspire.execution_runtime import get_runtime
    from solspire.project_manager import get_project_manager
    runtime = get_runtime(); pm = get_manager()
    ...
    return {"version": {"console": "1.0", "kernel": "0.1", "codex_app": "0.4", "publishing_app": "0.2", "research_app": "0.1"}, "providers": {"active": pm.active_provider(), "available": pm.list_providers(), "token_usage": pm.token_usage()}, "projects": {"active_count": len(projects)}, "executions": {"total": len(executions), "active": sum(1 for ex in executions if ex["status"] in ("running", "paused")), "by_status": _count_by_status(executions)}, "milestone": 1, "phase": 1}
...
async def update_project(project_id: str, body: UpdateProjectRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    import time as _time, sqlite3, os
    db_path = os.environ.get("SOLSPIRE_PROJECTS_DB", "data/solspire_projects.db")
    fields, vals = [], []
    if body.name is not None: fields.append("name=?"); vals.append(body.name.strip())
    if body.status is not None: fields.append("status=?"); vals.append(body.status)
    if body.description is not None:
        import json
        from solspire.project_manager import get_project_manager
        p = get_project_manager().load(project_id); p.metadata["description"] = body.description
    ...
    with sqlite3.connect(db_path) as conn: conn.execute(f"UPDATE projects SET {', '.join(fields)} WHERE id=?", vals)
    return {"ok": True}
...
async def project_list_conversations(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    ...
    items = list_conversations(project_id); return {"conversations": items, "count": len(items)}
@router.post("/projects/{project_id}/conversations")
async def project_create_conversation(project_id: str, body: CreateConversationRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_archive_conversation(project_id: str, conv_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import archive_conversation
    archive_conversation(conv_id); return {"ok": True}
@router.post("/projects/{project_id}/conversations/{conv_id}/messages")
async def project_append_message(project_id: str, conv_id: str, body: AppendMessageRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_create_file(project_id: str, body: CreateFileRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_get_file(project_id: str, file_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_update_file(project_id: str, file_id: str, body: UpdateFileRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import update_file
    return update_file(file_id, body.content, body.name)
@router.delete("/projects/{project_id}/files/{file_id}")
async def project_delete_file(project_id: str, file_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import delete_file
    if not delete_file(file_id): raise HTTPException(status_code=404, detail="File not found")
    return {"ok": True}
@router.post("/projects/{project_id}/files/upload")
async def project_upload_file(project_id: str, request: Request, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    content_type = request.headers.get("content-type", "")
    if not content_type.startswith("multipart/form-data"): raise HTTPException(status_code=400, detail="Expected multipart/form-data")
    body = await request.body(); boundary = content_type.split("boundary=")[-1].strip('"'); file_name = "upload"; raw: bytes = b""
    for part in body.split(("--" + boundary).encode()):
        if b"Content-Disposition" not in part: continue
        hdr_end = part.find(b"\r\n\r\n")
        if hdr_end == -1: continue
        headers = part[:hdr_end].decode("utf-8", errors="ignore"); content = part[hdr_end + 4:]
        ...
        if 'filename="' in headers:
            ...
            raw = content
    if not raw: raise HTTPException(status_code=400, detail="No file provided")
    from kernel.doc_extract import extract_text, make_label
    extracted_text, mime_type = extract_text(file_name, raw)
    if not extracted_text.strip(): raise HTTPException(status_code=400, detail="Could not extract any text from the uploaded file.")
    from solspire.project_store import create_file
    stored = create_file(project_id, file_name, extracted_text, mime_type)
    try:
        from knowledge.pipeline import ingest
        ingest(title=make_label(file_name), content=extracted_text, note_type="document", tags=["solspire", "project", "file", project_id], auto_tag=True, auto_embed=True, auto_link=True, user_id=user["uid"])
    except Exception: pass
    return {"ok": True, "file": stored, "file_name": file_name, "chars": len(extracted_text), "message": f"'{file_name}' attached to the project and ingested into Knowledge OS."}
...
async def project_link_repo(project_id: str, body: LinkRepoRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_unlink_repo(project_id: str, repo_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import unlink_repository
    if not unlink_repository(repo_id): raise HTTPException(status_code=404, detail="Repository not found")
    return {"ok": True}
@router.get("/projects/{project_id}/tasks")
async def project_list_tasks(project_id: str, status: str | None = None, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import list_tasks
    return {"tasks": list_tasks(project_id, status)}
@router.post("/projects/{project_id}/tasks")
async def project_create_task(project_id: str, body: CreateTaskRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_update_task(project_id: str, task_id: str, body: UpdateTaskRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import update_task
    return update_task(task_id, title=body.title, description=body.description, status=body.status, assigned_to=body.assigned_to, priority=body.priority)
@router.delete("/projects/{project_id}/tasks/{task_id}")
async def project_delete_task(project_id: str, task_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import delete_task
    if not delete_task(task_id): raise HTTPException(status_code=404, detail="Task not found")
    return {"ok": True}
@router.get("/projects/{project_id}/memory")
async def project_list_memory(project_id: str, q: str = "", user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_add_memory(project_id: str, body: AddMemoryRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_update_memory(project_id: str, mem_id: str, body: UpdateMemoryRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_delete_memory(project_id: str, mem_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import delete_memory
    if not delete_memory(mem_id): raise HTTPException(status_code=404, detail="Memory entry not found")
    return {"ok": True}
@router.get("/projects/{project_id}/events")
async def project_list_events(project_id: str, event_type: str | None = None, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import list_events
    return {"events": list_events(project_id, event_type)}
@router.post("/projects/{project_id}/run")
async def project_run(project_id: str, body: RunRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    """Run the canonical pipeline, then append the project activity event."""
    from solspire.project_store import log_event
    result = _execute_run(body, user)
    log_event(project_id, "workflow_run", f"⟐ {result['intent']}: {body.request[:80]}", {"status": result["execution"]["status"], "elapsed_ms": result["elapsed_ms"]})
    return result
...
async def project_weaver_capabilities(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_weaver_context_route(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
...
async def project_weaver_analyze(project_id: str, body: WeaverAnalyzeRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
    ...
    try: log_event(project_id, "weaver_analyze", f"Weaver analyze: {(body.objective or '')[:80]}", {"executed": False, "execution": "LOCKED", "scope": (result.get("scope") or {}).get("status")})
    except Exception: pass
    return result

@router.get("/projects/{project_id}/weaver/validation")
async def project_weaver_validation(project_id: str, scenario: str | None = None, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_weaver_knowledge_summary(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import list_memory, list_files, list_repositories, list_tasks, list_events
    mem, files, repos, tasks, events = list_memory(project_id), list_files(project_id), list_repositories(project_id), list_tasks(project_id), list_events(project_id)
    return {"project_id": project_id, "knowledge_os": {"memory_items": len(mem), "files": len(files), "repositories": len(repos), "tasks": len(tasks), "events": len(events)}, "graph": {"status": "NOT_A_SEPARATE_STORE", "note": "No second knowledge graph introduced. Use existing project relations via store tables."}, "embeddings": {"status": "NOT_A_SEPARATE_STORE", "note": "No second embedding store introduced in W4."}, "authorization": {"PassSpec": "NONE", "PatchApproval": "NONE", "Execution": "LOCKED", "note": "Knowledge summary is read-only project metadata."}}

@router.get("/projects/{project_id}/knowledge")
async def project_knowledge_os(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_knowledge_graph(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
...
async def project_knowledge_embeddings(project_id: str, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_knowledge import build_knowledge_summary
...
async def project_knowledge_search(project_id: str, body: dict[str, Any], user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_store import list_memory, list_files, list_tasks
    q = str((body or {}).get("q") or "").strip().lower(); hits = []
    if not q: return {"project_id": project_id, "q": q, "hits": [], "note": "empty query"}
    for m in list_memory(project_id) or []:
        if q in f"{m.get('title','')} {m.get('content','')}".lower(): hits.append({"type": "memory", "id": m.get("id"), "title": m.get("title"), "classification": "SOURCE-BACKED"})
    for f in list_files(project_id) or []:
        if q in str(f.get("name") or "").lower(): hits.append({"type": "file", "id": f.get("id"), "name": f.get("name"), "classification": "SOURCE-BACKED"})
    for t in list_tasks(project_id) or []:
        if q in f"{t.get('title','')} {t.get('description','')}".lower(): hits.append({"type": "task", "id": t.get("id"), "title": t.get("title"), "classification": "SOURCE-BACKED"})
    return {"project_id": project_id, "q": q, "hits": hits[:50], "note": "Keyword search over project_store. Not semantic vector retrieval.", "authorization": {"Execution": "LOCKED"}}
...
async def project_weaver_execution_readiness(project_id: str, body: WeaverReadinessRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
...
async def project_weaver_bind_pass_spec(project_id: str, body: WeaverPassSpecRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
    from solspire.weaver_bridge import project_bind_pass_spec
    p = get_project_manager().load(project_id)
    if not p: raise HTTPException(status_code=404, detail="Project not found")
    pdata = p.to_dict() if hasattr(p, "to_dict") else dict(p.__dict__)
    return project_bind_pass_spec(pdata, body.patch, pass_id=body.pass_id, objective=body.objective, allowed_paths=body.allowed_paths, required_tests=body.required_tests)

@router.post("/projects/{project_id}/weaver/execution/approval")
async def project_weaver_bind_approval(project_id: str, body: WeaverApprovalRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
...
async def project_weaver_execute(project_id: str, body: WeaverExecuteRequest, user: dict = Depends(require_project_owner)) -> dict[str, Any]:
    from solspire.project_manager import get_project_manager
