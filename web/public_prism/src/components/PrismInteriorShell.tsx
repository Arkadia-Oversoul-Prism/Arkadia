...
const SECONDARY = [
  ...
  { key: 'weaver', label: 'Weaver', tone: '#B08DE8' },
  { key: 'engineering-lab', label: 'Engineering Lab', tone: '#00D4AA' },
] as const;
...
function IdentityPersistence({ displayName, role, username, sigil, nodeKey }: { displayName: string; role: string; username: string; sigil: string; nodeKey: string }) {
  return <section aria-label="Identity persistence" data-testid="identity-persistence" style={{ width: '100%', maxWidth: '100vw', boxSizing: 'border-box', padding: '10px 16px 9px', borderBottom: '1px solid rgba(255,255,255,.07)', background: 'rgba(8,9,17,.94)', color: '#E9E7DF' }}><div style={{ maxWidth: 1180, width: '100%', minWidth: 0, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10 }}><div style={{ width: 30, height: 30, flex: '0 0 auto', borderRadius: 10, display: 'grid', placeItems: 'center', background: 'rgba(201,168,76,.07)', border: '1px solid rgba(201,168,76,.24)', color: '#C9A84C', fontFamily: 'Georgia,serif' }}>{sigil}</div><div style={{ minWidth: 0, flex: 1 }}><div style={{ display: 'flex', gap: 8, alignItems: 'baseline', minWidth: 0, flexWrap: 'nowrap' }}><span style={{ font: '600 10px Inter,system-ui,sans-serif', letterSpacing: '.12em', textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{displayName}</span><span style={{ font: '8px Inter,system-ui,sans-serif', color: 'rgba(233,231,223,.28)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{role}</span>{username && <span style={{ font: '8px ui-monospace,monospace', color: 'rgba(201,168,76,.45)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{username}</span>}</div><div style={{ font: '7px ui-monospace,monospace', color: 'rgba(233,231,223,.2)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{nodeKey}</div></div><div style={{ flexShrink: 0, font: '7px ui-monospace,monospace', letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(0,212,170,.55)' }}>PRISM · AUTHENTICATED</div></div></section>;
}
...
export default function PrismInteriorShell({ currentView, onNavigate, children }: Props) {
  ...
  if (solariunSurface) {
    return <div data-testid="prism-interior-shell" style={{ minHeight: '100vh', width: '100%', maxWidth: '100vw', overflowX: 'hidden', background: '#0C0D18', color: '#E9E7DF' }}><IdentityPersistence displayName={displayName} role={role} username={username} sigil={sigil} nodeKey={nodeKey} /><NovaNetRail currentView={currentView} onNavigate={onNavigate} />{children}</div>;
  }
