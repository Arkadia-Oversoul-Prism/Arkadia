...
function UserSection({ onNavigate, onClose }: { onNavigate: (v: View) => void; onClose: () => void }) {
  ...
  return <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, padding: '0 16px 10px' }}>{[{ label: 'Profile', view: 'account' as View }, { label: 'Solariun', view: 'solspire' as View }, { label: 'ReasoMate', view: 'reasomate' as View }, { label: 'Echo Field', view: 'solspire' as View }, { label: 'Encyclopedia', view: 'encyclopedia' as View }, { label: 'Offerings', view: 'offerings' as View }].map(item => <button key={item.label} onClick={() => { onNavigate(item.view); onClose(); }} style={{ padding: '8px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, color: 'rgba(232,232,232,0.45)', fontFamily: 'sans-serif', fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>{item.label}</button>)}</div>
