...
const ArkadiaNavigation: React.FC<NavProps> = ({ currentView, onNavigate, children }) => {
  ...
  return (
    <div className="relative min-h-screen" style={{ backgroundColor: '#0C0D18' }}>
      ...
      <nav className="fixed top-0 left-0 w-full z-40" style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '0 16px', height: 52, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)',
        borderBottom: '1px solid rgba(201,168,76,0.18)', backgroundColor: 'rgba(12,13,24,0.94)',
      }}>
        <button onClick={() => handleNavigate('home')} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          ...
          <span style={{ fontFamily: 'serif', fontSize: 10, letterSpacing: '0.38em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.90)', marginLeft: 2 }}>ARKADIA</span>
        </button>
        <AnimatePresence mode="wait">
          <motion.span key={currentView} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -3 }}
            transition={{ duration: 0.25 }}
            style={{ fontFamily: 'sans-serif', fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(232,232,232,0.62)', position: 'absolute', left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none', whiteSpace: 'nowrap' }}>
            {currentLabel}
          </motion.span>
        </AnimatePresence>
        <button onClick={() => setDrawerOpen(o => !o)} style={{
          ...
          borderRadius: 9, cursor: 'pointer', transition: 'all 0.2s', flexShrink: 0,
      ...
      <AnimatePresence>
        {drawerOpen && (
          <motion.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 45, background: 'rgba(2,3,8,0.65)', backdropFilter: 'blur(4px)' }}
          />
        )}
      </AnimatePresence>
      <AnimatePresence>
        {drawerOpen && (
          <motion.div key="drawer" initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px', height: 52, borderBottom: '1px solid rgba(201,168,76,0.16)', flexShrink: 0 }}>
              ...
              <button onClick={() => setDrawerOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(232,232,232,0.58)', fontSize: 16 }}>\u2715</button>
            </div>
                      ...
                      {visibleItems.map(item => {
                        ...
                        return (
                          <button key={item.view} onClick={() => handleNavigate(item.view)}
                            ...
                            <span style={{ fontSize: 13, flexShrink: 0, width: 22, textAlign: 'center', color: active ? item.color : 'rgba(232,232,232,0.52)' }}>{item.sigil}</span>
                            <div style={{ flex: 1, overflow: 'hidden' }}>
