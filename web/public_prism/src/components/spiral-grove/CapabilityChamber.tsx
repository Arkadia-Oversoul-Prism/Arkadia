...
export default function CapabilityChamber({ capability, state, prerequisites, learningPath, onBack, onOpenCapability }: CapabilityChamberProps) {
  const pathIds = learningPath?.capability_ids || []
  const pathIndex = pathIds.indexOf(capability.id)
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} data-testid="capability-chamber" style={{ border: '1px solid rgba(0,212,170,.20)', borderRadius: 18, background: 'linear-gradient(145deg, rgba(10,18,28,.96), rgba(7,10,18,.94))', overflow: 'hidden' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <button type="button" onClick={onBack} style={buttonStyle}>{'← Return to capability map'}</button>
      ...
      <div style={{ padding: 22 }}>
        <p style={eyebrow}>Enter chamber</p>
        <h2 style={heading}>{capability.name}</h2>
        <p style={description}>{capability.description}</p>
        ...
        <div style={grid}>
          ...
          <Panel title="Current learner state"><strong style={{ fontFamily: 'sans-serif', fontSize: 12, color: C.teal }}>{state.status.replaceAll('_', ' ')}</strong><p style={copy}>{state.next_recommended_action || 'Choose a practical next step.'}</p>{state.confidence !== null && <p style={meta}>Confidence: {Math.round(state.confidence * 100)}%</p>}</Panel>
          <Panel title="Prerequisites">{prerequisites.length ? prerequisites.map(item => <button key={item.id} type="button" onClick={() => onOpenCapability(item.id)} data-testid={`prerequisite-${item.slug}`} style={pathButton}><span style={{ fontFamily: 'sans-serif', fontSize: 10, color: C.text }}>{item.name}</span><span style={{ display: 'block', ...meta }}>Level {item.level} · open chamber →</span></button>) : <p style={copy}>No prerequisite capabilities. This is an entry point.</p>}</Panel>
        ...
        <div data-testid="learning-path-panel" style={pathPanel}>
          ...
          {learningPath ? <>
            <p style={pathReason}>{learningPath.reason}</p>
            <div data-testid="learning-path-steps" style={{ display: 'grid', gap: 7, marginTop: 12 }}>
              {pathIds.map((id, index) => {
                const item = [capability, ...prerequisites].find(candidate => candidate.id === id)
                if (!item) return null
                const isCurrent = id === capability.id
                const isNext = id === nextId
                return <div key={id} style={{ display: 'grid', gridTemplateColumns: '28px 1fr auto', gap: 9, alignItems: 'center' }}>
                  <span aria-hidden="true" style={{ width: 24, height: 24, display: 'grid', placeItems: 'center', borderRadius: 7, border: `1px solid ${isNext ? 'rgba(0,212,170,.40)' : 'rgba(255,255,255,.09)'}`, color: isNext ? C.teal : C.dim, fontFamily: 'sans-serif', fontSize: 8 }}>{index + 1}</span>
                  <button type="button" onClick={() => onOpenCapability(id)} data-testid={`path-step-${item.slug}`} style={{ ...pathButton, padding: '7px 9px', border: isCurrent ? '1px solid rgba(0,212,170,.18)' : '1px solid rgba(255,255,255,.05)' }}><span style={{ color: isCurrent || isNext ? C.teal : C.text }}>{item.name}</span><span style={{ display: 'block', ...meta }}>{isCurrent ? 'Current chamber' : isNext ? 'Recommended next' : 'Prerequisite context'}</span></button>
                  {isNext && <span style={{ fontFamily: 'sans-serif', fontSize: 8, color: C.teal, textTransform: 'uppercase', letterSpacing: '.1em' }}>Next</span>}
                </div>
              })}
            </div>
            ...
            <p style={boundary}>SG-03 path projection · completion requires evidence. This interaction only navigates the existing path and does not generate exercises or evidence.</p>
...
const pathButton: React.CSSProperties = { width: '100%', textAlign: 'left', background: 'rgba(255,255,255,.018)', border: '1px solid rgba(255,255,255,.05)', borderRadius: 8, padding: '8px 9px', cursor: 'pointer', color: C.text, fontFamily: 'sans-serif', fontSize: 9 }
