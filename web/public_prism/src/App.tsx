...
function PortalDoor({ label, sub, color, sigil, onClick, delay, locked }: { label: string; sub: string; color: string; sigil: string; onClick: () => void; delay: number; locked?: boolean; }) {
  ...
  return <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.55 }} onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ padding: '14px 16px', background: hovered ? 'rgba(14,17,32,0.92)' : 'rgba(14,17,32,0.72)', border: `1px solid ${hovered ? color + '55' : 'rgba(0,212,170,0.16)'}`, borderRadius: '10px', cursor: 'pointer', transition: 'all 0.22s', display: 'flex', alignItems: 'center', gap: '13px', opacity: locked ? 0.45 : 1 }}><span style={{ fontSize: '16px', flexShrink: 0, width: '24px', textAlign: 'center' }}>{sigil}</span><div style={{ flex: 1 }}><p style={{ fontFamily: 'sans-serif', fontSize: '10px', letterSpacing: '0.22em', textTransform: 'uppercase', color: hovered ? color : 'rgba(232,232,232,0.72)', margin: '0 0 3px', fontWeight: 500 }}>{label}</p><p style={{ fontFamily: 'sans-serif', fontSize: '11px', color: 'rgba(232,232,232,0.38)', margin: 0, lineHeight: 1.4 }}>{sub}</p></div><span style={{ color: hovered ? color : 'rgba(255,255,255,0.22)', fontSize: '11px' }}>{locked ? 'x' : '>'}</span></motion.div>;
}
function Home({ onNavigate }: { onNavigate: (v: View) => void }) {
  const { isAuthenticated } = useAuth();
  return <div className="min-h-screen w-full relative"><div className="aurora-bg" /><div className="page-column relative z-10 pt-10 pb-16 flex flex-col"><div style={{ marginBottom: '26px', display: 'flex', justifyContent: 'center' }}><FieldPulse /></div><h1 style={{ fontFamily: 'serif', fontSize: '52px', letterSpacing: '0.18em', textAlign: 'center', color: '#C9A84C', marginBottom: '10px', lineHeight: 1 }}>ARKADIA</h1><p style={{ fontFamily: 'serif', fontSize: '17px', lineHeight: '1.5', color: 'rgba(232,232,232,0.78)', margin: '0 0 22px', textAlign: 'center', maxWidth: '28em', alignSelf: 'center' }}>{isAuthenticated ? 'Your private workspace is open — conversations, notes, and projects stay with you.' : 'A place to think, remember, and build — with AI that keeps your thread.'}</p><div style={{ marginBottom: '10px' }}><button onClick={() => onNavigate(isAuthenticated ? 'commune' : 'gate')} data-testid="button-home-oracle" style={{ width: '100%', padding: '17px', background: 'linear-gradient(135deg, rgba(0,212,170,0.16), rgba(0,212,170,0.06))', border: '1px solid rgba(0,212,170,0.5)', borderRadius: '11px', color: '#00D4AA', fontFamily: 'sans-serif', fontSize: '11px', letterSpacing: '0.22em', textTransform: 'uppercase', cursor: 'pointer' }}>{isAuthenticated ? 'Continue with the Oracle' : 'Enter Arkadia'}</button></div><div style={{ marginBottom: '18px' }}><button onClick={() => onNavigate(isAuthenticated ? 'solspire' : 'gate')} data-testid="button-home-private" style={{ width: '100%', padding: '13px', background: 'rgba(14,17,32,0.55)', border: '1px solid rgba(201,168,76,0.28)', borderRadius: '11px', color: 'rgba(201,168,76,0.85)', fontFamily: 'sans-serif', fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer' }}>{isAuthenticated ? 'Open your private field' : 'Sign in / Create Node'}</button></div><div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '30px' }}><PortalDoor label="Oracle" sub="ARKANA · Pattern intelligence · Live commune" color="#00D4AA" sigil="*" onClick={() => onNavigate(isAuthenticated ? 'commune' : 'gate')} delay={1.12} /><PortalDoor label="NovaNet — Nexus Hub" sub="Public Feed · ReasoMate · Offerings · Stellar Cartography" color="#6A9FD8" sigil="o" onClick={() => onNavigate(isAuthenticated ? 'novanet' : 'gate')} delay={1.13} /><PortalDoor label="SolSpire Console" sub="Echo Field Matrix · Spiral Codex · Personal Codex · Projects · Knowledge" color="#C9A84C" sigil="o" onClick={() => onNavigate(isAuthenticated ? 'solspire' : 'gate')} delay={1.14} /><PortalDoor label="SCI" sub="Spiral Command Interface · operator shell" color="#00D4AA" sigil="#" onClick={() => onNavigate(isAuthenticated ? 'sci' : 'gate')} delay={1.15} /><PortalDoor label="ReasoMate" sub="Arkana messenger · continuous conversation" color="#6A9FD8" sigil="+" onClick={() => onNavigate(isAuthenticated ? 'reasomate' : 'gate')} delay={1.16} /><PortalDoor label="Future Skills Lab" sub="Free 60-minute practical capability challenge" color="#00D4AA" sigil="→" onClick={() => onNavigate('challenge')} delay={1.17} /><PortalDoor label="Offerings" sub="IMS Sessions · Products · AIC Diagnostic" color="#00D4AA" sigil="*" onClick={() => onNavigate('offerings')} delay={1.18} /></div>{!isAuthenticated && <div style={{ marginBottom: '16px', textAlign: 'center' }}><button onClick={() => onNavigate('gate')} style={{ background: 'none', border: 'none', color: 'rgba(0,212,170,0.35)', fontFamily: 'sans-serif', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }} data-testid="button-home-login">Sign in / create your node</button></div>}</div></div>;
}
function AppInner() {
