  ...
  return (
    <div className="min-h-screen w-full relative">
      ...
      <div className="page-column relative z-10 pt-10 pb-16 flex flex-col">
        ...
        <h1 style={{ fontFamily: 'serif', fontSize: '52px', letterSpacing: '0.18em', textAlign: 'center', color: '#C9A84C', marginBottom: '10px' }}>ARKADIA</h1>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '30px' }}>
          <PortalDoor label="Oracle" sub="ARKANA" color="#00D4AA" sigil="\u27d0" onClick={() => onNavigate('commune')} delay={0.1} />
          <PortalDoor label="NovaNet \u2014 Nexus Hub" sub="Public Feed" color="#6A9FD8" sigil="\u25c9" onClick={() => onNavigate('novanet')} delay={0.12} />
          <PortalDoor label="SolSpire Console" sub="Projects \u00b7 Knowledge" color="#C9A84C" sigil="\u25c9" onClick={() => onNavigate('solspire')} delay={0.14} />
          <PortalDoor label="SCI" sub="Spiral Command Interface" color="#00D4AA" sigil="\u2318" onClick={() => onNavigate('sci')} delay={0.15} />
          <PortalDoor label="ReasoMate" sub="Messenger" color="#6A9FD8" sigil="\u2727" onClick={() => onNavigate('reasomate')} delay={0.16} />
          <PortalDoor label="Offerings" sub="IMS \u00b7 Products" color="#00D4AA" sigil="\u2726" onClick={() => onNavigate('offerings')} delay={0.18} />
        </div>
        {!isAuthenticated && (
          <button onClick={() => onNavigate('login')} style={{ background: 'none', border: 'none', color: 'rgba(0,212,170,0.35)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>\ud83d\udd10 Sign in</button>
        )}
      </div>
    </div>
  );
