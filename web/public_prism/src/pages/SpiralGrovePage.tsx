...
import { AIS_CAPABILITIES, GROVE_DOMAINS, INITIAL_LEARNER_STATES, learningPathFor, prerequisitesFor, type GroveCapability, type LearnerCapabilityState } from '../data/spiralGroveCatalog'
...
function readDomainState(): Record<string, GroveGatewayState> {
  ...
  try { const parsed = JSON.parse(window.localStorage.getItem(DOMAIN_STATE_KEY) || '{}'); return parsed && typeof parsed === 'object' ? parsed : {} } catch { return {} }
}
...
function CapabilityCard({ capability, state, onEnter }: { capability: GroveCapability; state: LearnerCapabilityState; onEnter: () => void }) {
  ...
  return <button type="button" onClick={onEnter} data-testid={`capability-${capability.slug}`} style={{ textAlign: 'left', width: '100%', padding: 15, cursor: 'pointer', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, color: C.text }}>
...
export default function SpiralGrovePage() {
  ...
  const learningPath = selected ? learningPathFor(selected.id, selectedState) : null
  ...
  const openCapability = (capabilityId: string) => {
    const capability = AIS_CAPABILITIES.find(item => item.id === capabilityId)
    if (!capability) return
    setSelectedId(capabilityId)
    setActiveDomain(capability.domain)
    setChamberOpen(true)
  }
  const exitChamber = () => setChamberOpen(false)

  return <div style={{ paddingBottom: 40 }} data-testid="spiral-grove">
    ...
    {chamberOpen && selected ? <CapabilityChamber capability={selected} state={selectedState} prerequisites={prerequisitesFor(selected.id)} learningPath={learningPath} onBack={exitChamber} onOpenCapability={openCapability} /> : <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 14 }}><section><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 8 }}>{domainCapabilities.map(capability => <CapabilityCard key={capability.id} capability={capability} state={states.find(item => item.capability_id === capability.id) || INITIAL_LEARNER_STATES[0]} onEnter={() => openCapability(capability.id)} />)}</div></section></div>}
  </div>
}
