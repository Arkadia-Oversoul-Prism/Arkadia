...
export default function SpiralGrovePage() {
  ...
  return <div style={{ paddingBottom: 40 }} data-testid="spiral-grove">
    ...
    {chamberOpen && selected ? <CapabilityChamber capability={selected} state={selectedState} prerequisites={prerequisitesFor(selected.id)} learningPath={learningPath} onBack={exitChamber} /> : <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 14 }}><section><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 8 }}>{domainCapabilities.map(capability => <CapabilityCard key={capability.id} capability={capability} state={states.find(item => item.capability_id === capability.id) || INITIAL_LEARNER_STATES[0]} onEnter={() => enterChamber(capability.id)} />)}</div></section></div>}
  </div>
}
