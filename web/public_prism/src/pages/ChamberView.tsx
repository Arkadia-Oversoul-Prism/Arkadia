/**
 * ChamberView — Full-screen chamber reading experience.
 * Shared between Crystal Tribune (NexusSpiralCodex) and any future surface.
 *
 * Extracted from EncyclopediaGalactica so the Crystal Tribune can unify
 * Spiral Codex navigation with Encyclopedia chapter reading.
 */
...
export type ChamberState = 'dormant' | 'explored' | 'integrated'
...
export function loadChamberStates(): Record<number, ChamberState> {
  try { return JSON.parse(localStorage.getItem(LS_STATES) ?? '{}') } catch { return {} }
}
export function saveChamberStates(s: Record<number, ChamberState>) {
  localStorage.setItem(LS_STATES, JSON.stringify(s))
}
export function loadChamberReflections(): Record<number, string> {
  try { return JSON.parse(localStorage.getItem(LS_REFLECTIONS) ?? '{}') } catch { return {} }
}
export function saveChamberReflections(r: Record<number, string>) {
...
function ReadingProgress({ color, scrollRef }: { color: string; scrollRef: React.RefObject<HTMLDivElement> }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      setProgress(scrollHeight <= clientHeight ? 1 : Math.min(1, scrollTop / (scrollHeight - clientHeight)))
    }
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'rgba(255,255,255,0.04)' }}>
      <motion.div style={{ height: '100%', background: color, transformOrigin: 'left', boxShadow: `0 0 8px ${color}60` }}
        animate={{ scaleX: progress }} transition={{ duration: 0.08 }} />
    </div>
  )
}
...
export function ChamberCodexFeed({ chamber }: { chamber: Chamber }) {
  const [open, setOpen] = useState(false)
  const { data } = useQuery<CodexResponse>({ queryKey: ['codex-egalactica'], queryFn: api.codex, staleTime: 5 * 60_000, enabled: open })
  const related = useMemo(() => {
    if (!data?.scrolls) return []
    return Object.values(data.scrolls)
      .filter(s => { const text = `${s.label ?? ''} ${s.description ?? ''} ${s.preview ?? ''}`.toLowerCase(); return chamber.keywords.some(kw => text.includes(kw)) })
      .slice(0, 5)
    ...
    <div style={{ marginTop: 8 }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer',
          padding: '12px 0', fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.3em',
          textTransform: 'uppercase', color: `${chamber.color}70` }}>
        <motion.span animate={{ rotate: open ? 90 : 0 }} transition={{ duration: 0.18 }}
          style={{ fontSize: 10, display: 'inline-block' }}>›</motion.span>
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingBottom: 12 }}>
              {related.length === 0 && <p style={{ fontFamily: 'sans-serif', fontSize: 11.5, color: 'rgba(232,232,232,0.3)', margin: 0, fontStyle: 'italic' }}>{data ? 'No related scrolls indexed.' : 'Loading corpus…'}</p>}
              {related.map(s => (
                <div key={s.id} style={{ padding: '12px 16px', background: `${chamber.color}06`, border: `1px solid ${chamber.color}18`, borderLeft: `2px solid ${chamber.color}45`, borderRadius: '0 10px 10px 0' }}>
...
export function ChapterIndex({ current, states, onSelect, onClose }: {
  current: number; states: Record<number, ChamberState>; onSelect: (n: number) => void; onClose: () => void
}) {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
      transition={{ duration: 0.2 }}
      style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(2,3,8,0.92)', backdropFilter: 'blur(24px)',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: 560 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 8, letterSpacing: '0.45em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.35)', margin: '0 0 4px' }}>Encyclopedia Galactica</p>
            <h2 style={{ fontFamily: 'serif', fontSize: 20, color: 'rgba(232,232,232,0.82)', margin: 0, letterSpacing: '0.05em', fontWeight: 400 }}>Chapter Index</h2>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: 'rgba(232,232,232,0.4)', cursor: 'pointer', padding: '8px 14px', fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.2em' }}>✕ Close</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8 }}>
          {CHAMBERS.map(ch => {
            const st = states[ch.num] ?? 'dormant'; const active = ch.num === current
            return (
              <motion.button key={ch.num} onClick={() => { onSelect(ch.num); onClose() }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                style={{ padding: '14px 14px 12px', background: active ? `${ch.color}14` : st === 'integrated' ? `${ch.color}07` : 'rgba(255,255,255,0.025)',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                  <span style={{ fontSize: 14, color: active ? ch.color : st !== 'dormant' ? `${ch.color}80` : 'rgba(232,232,232,0.25)' }}>{ch.sigil}</span>
                  <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 8, letterSpacing: '0.22em', textTransform: 'uppercase', color: active ? `${ch.color}80` : 'rgba(232,232,232,0.22)' }}>{ROMAN[ch.num - 1]}</span>
                ...
                <p style={{ fontFamily: 'sans-serif', fontSize: 10.5, color: active ? ch.color : 'rgba(232,232,232,0.55)', margin: '0 0 3px', lineHeight: 1.35, fontWeight: active ? 600 : 400 }}>{ch.chapterTitle}</p>
                <p style={{ fontFamily: 'sans-serif', fontSize: 9, color: 'rgba(232,232,232,0.22)', margin: 0, lineHeight: 1.3 }}>{ch.chamberName}</p>
              </motion.button>
            )
          })}
        </div>
...
export default function ChamberView({ chamber, states, reflections, onReturn, onNext, onPrev, onMarkIntegrated, onSaveReflection, onOpenIndex }: {
  chamber: Chamber; states: Record<number, ChamberState>; reflections: Record<number, string>
  onReturn: () => void; onNext: () => void; onPrev: () => void
  onMarkIntegrated: (num: number) => void; onSaveReflection: (num: number, text: string) => void
  onOpenIndex: () => void
}) {
  ...
  useEffect(() => { setReflection(reflections[chamber.num] ?? ''); setSaved(true); scrollRef.current?.scrollTo(0, 0) }, [chamber.num, reflections])
  useEffect(() => {
  ...
  return (
    <motion.div key={`chamber-${chamber.num}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {/* Top nav */}
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '12px 18px', background: 'rgba(3,4,8,0.90)', borderBottom: `1px solid ${chamber.color}14`,
        backdropFilter: 'blur(24px)', gap: 8 }}>
        <button onClick={onReturn} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer',
          fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase',
          color: `${chamber.color}65`, padding: '6px 8px', borderRadius: 7, flexShrink: 0 }}>
          ← Crystal
        </button>
        <div style={{ flex: 1, textAlign: 'center', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            ...
            <span style={{ padding: '2px 8px', background: `${chamber.color}10`, border: `1px solid ${chamber.color}25`, borderRadius: 999,
              fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.15em', textTransform: 'uppercase', color: `${chamber.color}70`, whiteSpace: 'nowrap' }}>
              {stateLabel}
            </span>
            <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 7.5, color: 'rgba(232,232,232,0.18)', whiteSpace: 'nowrap' }}>~{readMins}m</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
          <button onClick={onOpenIndex} style={{ padding: '6px 10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 7, color: 'rgba(232,232,232,0.35)', cursor: 'pointer', fontFamily: 'ui-monospace, monospace', fontSize: 8.5, letterSpacing: '0.18em', textTransform: 'uppercase' }}>⊞</button>
          <button onClick={onPrev} style={{ padding: '6px 11px', background: 'none', border: `1px solid ${chamber.color}18`, borderRadius: 7, color: `${chamber.color}60`, cursor: 'pointer', fontFamily: 'ui-monospace, monospace', fontSize: 11 }}>‹</button>
          <button onClick={onNext} style={{ padding: '6px 11px', background: 'none', border: `1px solid ${chamber.color}18`, borderRadius: 7, color: `${chamber.color}60`, cursor: 'pointer', fontFamily: 'ui-monospace, monospace', fontSize: 11 }}>›</button>
        </div>
      </div>
      ...
      <div ref={scrollRef} style={{ height: '100vh', overflowY: 'auto', paddingTop: 60 }}>
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '48px 24px 100px' }}>
          <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 8, letterSpacing: '0.38em', textTransform: 'uppercase', color: 'rgba(232,232,232,0.16)', margin: '0 0 16px', textAlign: 'center' }}>{chamber.part}</p>
          <div style={{ textAlign: 'center', marginBottom: 22 }}>
            <motion.div animate={{ opacity: [0.5,1,0.5], scale: [0.97,1.04,0.97] }} transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut' }}
              style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 80, height: 80, borderRadius: '50%',
                border: `1px solid ${chamber.color}30`, background: `${chamber.color}07`, boxShadow: `0 0 50px ${chamber.color}14, 0 0 100px ${chamber.color}06` }}>
          ...
          <div style={{ padding: '28px 32px', background: `linear-gradient(135deg, ${chamber.color}09 0%, ${chamber.color}04 100%)`,
            border: `1px solid ${chamber.color}20`, borderLeft: `3px solid ${chamber.color}65`, borderRadius: '0 14px 14px 0', marginBottom: 48 }}>
            <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.42em', textTransform: 'uppercase', color: `${chamber.color}45`, margin: '0 0 18px' }}>Opening Verse</p>
            {chamber.openingVerse.split('\n').map((line, i) => (
              <p key={i} style={{ fontFamily: 'serif', fontSize: 17, lineHeight: '2.0', color: line.trim() === '' ? undefined : 'rgba(232,232,232,0.78)', fontStyle: 'italic', margin: line.trim() === '' ? '6px 0' : '0' }}>
                {line.trim() === '' ? '\u00A0' : line}
              </p>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
            ...
            <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(232,232,232,0.18)', margin: 0, whiteSpace: 'nowrap' }}>Echoes of the Lost Aeons</p>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.04)' }} />
          </div>
          <div style={{ marginBottom: 52 }}>
            {chamber.excerpt.split('\n\n').map((para, i) => (
              <p key={i} style={{ fontFamily: 'serif', fontSize: 18, lineHeight: '2.05', color: 'rgba(232,232,232,0.72)', margin: '0 0 28px', letterSpacing: '0.01em' }}>{para}</p>
            ))}
          </div>
          <div style={{ padding: '28px 32px', background: 'rgba(255,255,255,0.015)', border: `1px solid ${chamber.color}14`, borderRadius: 14, marginBottom: 52, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: -10, left: 16, fontFamily: 'serif', fontSize: 120, color: `${chamber.color}07`, pointerEvents: 'none', lineHeight: 1 }}>"</div>
            <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.42em', textTransform: 'uppercase', color: `${chamber.color}40`, margin: '0 0 18px' }}>Closing Verse</p>
            {chamber.closingVerse.split('\n').map((line, i) => (
              <p key={i} style={{ fontFamily: 'serif', fontSize: 17.5, lineHeight: '1.95', color: line.trim() === '' ? undefined : `rgba(232,232,232,0.68)`, fontStyle: 'italic', margin: line.trim() === '' ? '8px 0' : '0' }}>
                {line.trim() === '' ? '\u00A0' : line}
              </p>
            ))}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 48 }}>
            {chamber.keywords.map(kw => (
              <span key={kw} style={{ padding: '4px 11px', background: `${chamber.color}08`, border: `1px solid ${chamber.color}18`, borderRadius: 999,
                fontFamily: 'ui-monospace, monospace', fontSize: 8.5, letterSpacing: '0.12em', textTransform: 'uppercase', color: `${chamber.color}55` }}>
                {kw}
              </span>
            ))}
          </div>
          <div style={{ height: 1, background: `linear-gradient(90deg, transparent, ${chamber.color}18, transparent)`, marginBottom: 40 }} />
          <div style={{ padding: '24px 26px', background: `${chamber.color}05`, border: `1px solid ${chamber.color}18`, borderRadius: 14, marginBottom: 36 }}>
            <p style={{ fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.4em', textTransform: 'uppercase', color: `${chamber.color}50`, margin: '0 0 14px' }}>Reflection Prompt</p>
            <p style={{ fontFamily: 'serif', fontSize: 17, lineHeight: '1.85', color: 'rgba(232,232,232,0.65)', fontStyle: 'italic', margin: '0 0 22px' }}>{chamber.reflectionPrompt}</p>
            <textarea value={reflection} onChange={e => { setReflection(e.target.value); setSaved(false) }}
              placeholder="Write here. This field holds your truth." rows={6}
              style={{ width: '100%', boxSizing: 'border-box', padding: '16px 18px', background: 'rgba(3,4,8,0.6)', border: `1px solid ${chamber.color}22`, borderRadius: 10, color: 'rgba(232,232,232,0.78)', fontFamily: 'serif', fontSize: 16, lineHeight: '1.85', outline: 'none', resize: 'vertical', marginBottom: 14, caretColor: chamber.color }} />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 8, color: 'rgba(232,232,232,0.2)', letterSpacing: '0.1em' }}>{reflection.trim() ? `${reflection.trim().split(/\s+/).length} words` : ''}</span>
              {!saved && <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 8, color: `${chamber.color}45`, letterSpacing: '0.1em' }}>unsaved</span>}
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {!saved && (
                <button onClick={() => { onSaveReflection(chamber.num, reflection); setSaved(true) }}
                  style={{ padding: '11px 22px', background: `${chamber.color}14`, border: `1px solid ${chamber.color}42`, borderRadius: 9, color: chamber.color, fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', cursor: 'pointer' }}>
                  Save Reflection
                </button>
              )}
              {saved && reflection.trim() && state !== 'integrated' && (
                <button onClick={() => onMarkIntegrated(chamber.num)}
                  style={{ padding: '11px 22px', background: `${chamber.color}09`, border: `1px solid ${chamber.color}30`, borderRadius: 9, color: `${chamber.color}85`, fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', cursor: 'pointer' }}>
                  ✦ Mark as Integrated
                </button>
              )}
              {state === 'integrated' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 20px', background: `${chamber.color}08`, border: `1px solid ${chamber.color}22`, borderRadius: 9 }}>
                  <motion.span animate={{ opacity: [0.5,1,0.5] }} transition={{ duration: 2.5, repeat: Infinity }} style={{ color: chamber.color }}>✦</motion.span>
                  <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: `${chamber.color}70` }}>Integrated</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginTop: 56, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.04)' }}>
            <button onClick={onPrev} style={{ padding: '14px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 11, color: 'rgba(232,232,232,0.42)', fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>‹ Prev</button>
            <button onClick={onReturn} style={{ padding: '14px', background: `${chamber.color}09`, border: `1px solid ${chamber.color}28`, borderRadius: 11, color: `${chamber.color}75`, fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>☥ Crystal</button>
            <button onClick={onNext} style={{ padding: '14px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 11, color: 'rgba(232,232,232,0.42)', fontFamily: 'ui-monospace, monospace', fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', cursor: 'pointer' }}>Next ›</button>
          </div>
          <p style={{ textAlign: 'center', fontFamily: 'ui-monospace, monospace', fontSize: 7.5, letterSpacing: '0.2em', color: 'rgba(232,232,232,0.12)', marginTop: 20 }}>← → arrow keys · esc to return</p>
        </div>
      </div>
    </motion.div>
  )
