...
const LEGACY_MAP: Record<string, SolSpireLens> = {field:'overview',codex:'knowledge',loops:'tasks',projects:'files',encyclopedia:'knowledge',goals:'overview',releases:'overview',jobs:'observatory',traces:'observatory',tools:'observatory',system:'settings'};
...
export default function SolSpireConsole({ onNavigate, initialSection = 'overview' }: { onNavigate?: (v: AppView) => void; initialSection?: LegacySection } = {}) {
  ...
  return <SolSpireExperience identity={resolveIdentity(user)} initialSection={(LEGACY_MAP[initialSection] || initialSection) as SolSpireLens} />;
}
